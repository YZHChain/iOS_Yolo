...
{
    //TODO: 复制发生野指针错误.当达到满 200 时,输入中文出现
//    if (YZHIsString(text)) {
        [super setText:text];
//    }
...
- (void)textDidChange:(NSNotification *)notification
{
    ...
    if (_maxLength != NSUIntegerMax && _maxLength != 0 && self.text.length > 0) {
        NSInteger currenTextLeng = [self.text yzh_calculateStringLeng];
        if (!self.markedTextRange && currenTextLeng > _maxLength) {
            !_maxHandler ?: _maxHandler(self); // 回调达到最大限制的Block.
            //暂时使用这种方式来进行截取,由于使用下面的方式截取会导致无限循环. 内存泄漏。
                self.text = [self.text substringToIndex:self.text.length - 1];
//             TODO:可能是截取导致.
//            if (self.text.length >= 200) {
////                self.text = [self.text substringToIndex:_maxLength]; // 截取最大限制字符数.
//            } else {
//               self.text = [self.text substringToIndex:self.text.length]; // 截取最大限制字符数.
//            }
